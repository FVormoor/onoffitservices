...
class syscoonFinanceinterface(models.Model):
    ...
    def export(self, mode=False, date_from=False, date_to=False, args=False):
        ...
        if mode == "datev_ascii_accounts":
            if not args:
                raise UserError(
                    _("There are not all parameters given, that are necessary")
                )
            ...
            if account_kind == "all":
                if account == "debit":
                    partner_ids = (
                        ...
                        .sudo()
                        .search(
                            [("parent_id", "=", False), ("debitor_number", "!=", False)]
                        )
                    )
                elif account == "credit":
                    partner_ids = (
            ...
            if not partner_ids:
                raise UserError(
                    _(
                        "There are no accounts to export with your export-settings. Please check them!"
                    )
                )
            else:
                export_header = False
            ...
            if csv:
                ...
                raise UserError(
                    _(
                        "Something went wrong, because a export file could not generated!"
                    )
                )
        else:
            return export_id
    ...
    def generate_export_partners(self, partner_ids, kind):
        ...
        for partner_id in partner_ids:
            ...
            for number in partner_numbers:
                if number:
                    if kind == "rewe":
                        converted_partner, account_id = self.generate_rewe_partner(
                            partner_id, number, self.export_template_accounts()
                        )
                    else:
                        converted_partner, account_id = self.generate_duo_partner(
    ...
    def generate_rewe_partner(self, partner_id, number, template):
        ...
        account_id = (
            self.env["account.account"].search([("code", "=", number[1:])]) or False
        )
        ...
        if partner_id.vat and partner_id.vat[:2] in (
            "BE",
            "BG",
            "DK",
            "DE",
        ...
        if partner_id.bank_ids:
            for bank in partner_id.bank_ids:
                ...
                template["Abw. Kontoinhaber %s" % bank_count] = (
                    bank.acc_holder_name or ""
                )
                if bank.acc_type == "iban":
                    ...
                    template["Länderkennzeichen %s" % bank_count] = (
        ...
        if (
            ...
            for line in partner_id.property_payment_term_id.line_ids:
                ...
                if (
                    line.value == "percent"
                    and not template["Skonto in Prozent (Debitor)"]
                ):
