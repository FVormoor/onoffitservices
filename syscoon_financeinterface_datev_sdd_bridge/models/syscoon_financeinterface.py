...
class syscoonFinanceinterface(models.Model):
    ...
    def generate_rewe_partner(self, partner_id, number, template):
        template, account_id = super(
            syscoonFinanceinterface, self
        ).generate_rewe_partner(partner_id, number, template)
    ...
    def generate_export_line(self, export_line, line):
        export_line, group = super(syscoonFinanceinterface, self).generate_export_line(
            export_line, line
        )
