...
class syscoonFinanceinterface(models.Model):
    ...
    def correct_amount(self, export_lines, amount_check, amount_real):
        ...
        for line in export_lines:
            ...
            if not (
                ...
                or counteraccount_id.user_type_id.type == "liquidity"
            ):
    ...
    def generate_export_line(self, export_line, line):
        ...
        if (
            ...
            export_line["Kurs"] = line.company_id.currency_id._convert(
                1.0, line.currency_id, line.company_id, line.date, round=False
            )
        ...
        if line.partner_id:
            ...
            if (
                line.account_id.user_type_id.type == "payable"
            ...
            if (
                line.move_id.export_account_counterpart.user_type_id.type == "payable"
                and partner.creditor_number
            ):
                export_line[
                    "Gegenkonto (ohne BU-Schlüssel)"
            ...
            if (
                line.move_id.export_account_counterpart.user_type_id.type
                == "receivable"
                and partner.debitor_number
            ):
                export_line[
            ...
            if line.partner_id.country_id:
                export_line["Land"] = line.partner_id.country_id.code
                export_line["EU-Land (Anzahlungen)"] = line.partner_id.country_id.code
        ...
        export_line["Belegdatum"] = self.convert_date(
            line.date, self.env.company.datev_voucher_date_format
        )
        if line.move_id.invoice_date and line.date != line.move_id.invoice_date:
            export_line["Belegdatum"] = self.convert_date(
                line.move_id.invoice_date, self.env.company.datev_voucher_date_format
            )
            export_line["Leistungsdatum"] = self.convert_date(line.date, "%d%m%Y")
        ...
        if line.move_id.invoice_payment_term_id.datev_payment_conditons_id:
            export_line[
                "Belegfeld 2"
            ] = line.move_id.invoice_payment_term_id.datev_payment_conditons_id
        export_line["Buchungstext"] = self.create_label(line)
        if line.company_id.datev_use_bedi and line.move_id.datev_bedi:
            export_line["Beleglink"] = 'BEDI "%s"' % line.move_id.datev_bedi
        ...
        if line.account_id.datev_vatid_required:
            if (
                line.move_id.partner_shipping_id
                and line.move_id.partner_shipping_id.vat
            ):
                export_line[
                    "EU-Mitgliedstaat u. UStIdNr"
    ...
    def create_doc_field(self, line, group):
        ...
        if (
            ...
            if line.move_id.asset_id and line.move_id.asset_id.original_move_line_ids:
                doc_field = line.move_id.asset_id.original_move_line_ids[
                    0
                ].move_id.datev_ref
        if line.move_id.reversed_entry_id:
            if (
                line.move_id.reversed_entry_id.asset_id
                and line.reversed_entry_id.move_id.asset_id.original_move_line_ids
            ):
                doc_field = (
                    line.move_id.reversed_entry_id.asset_id.original_move_line_ids[
                        0
                    ].move_id.datev_ref
                )
            else:
    ...
    def group_converted_move_lines(self, move_lines, group_journal, group):
        ...
        for ml in move_lines:
            if group_journal and group:
                if not any(
                    gl["Konto"] == ml["Konto"]
                    and gl["Gegenkonto (ohne BU-Schlüssel)"]
                    == ml["Gegenkonto (ohne BU-Schlüssel)"]
                    and gl["BU-Schlüssel"] == ml["BU-Schlüssel"]
                    and gl["Soll/Haben-Kennzeichen"] == ml["Soll/Haben-Kennzeichen"]
                    and gl["WKZ Umsatz"] == ml["WKZ Umsatz"]
                    ...
                    for gl in grouped_lines:
                        if (
                            ...
                            == ml["Gegenkonto (ohne BU-Schlüssel)"]
                            and gl["BU-Schlüssel"] == ml["BU-Schlüssel"]
                            and gl["Soll/Haben-Kennzeichen"]
                            == ml["Soll/Haben-Kennzeichen"]
                        ):
                            if (
                                gl["Basis-Umsatz"]
                                or gl["Basis-Umsatz"] == 0.0
                                and ml["Basis-Umsatz"]
                            ):
                                gl["Basis-Umsatz"] += ml["Basis-Umsatz"]
                            if (
                                gl["Umsatz (ohne Soll/Haben-Kz)"]
                                or gl["Umsatz (ohne Soll/Haben-Kz)"] == 0.0
                                and ml["Umsatz (ohne Soll/Haben-Kz)"]
                            ):
                                gl["Umsatz (ohne Soll/Haben-Kz)"] += ml[
                                    "Umsatz (ohne Soll/Haben-Kz)"
                                ]
                            break
            else:
    ...
    def generate_export_header(self, header, date_from, date_to):
        if int(self.env.company.fiscalyear_last_month) == 12:
            ...
            fy_start = (
                ...
                + "{:02d}".format(int(self.env.company.fiscalyear_last_month) + 1)
                + "01"
            )
