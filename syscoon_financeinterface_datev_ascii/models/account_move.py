...
class AccountMove(models.Model):
    ...
    datev_ref = fields.Char(
        "DATEV Ref", compute="_compute_datev_ref", search="_search_datev_ref"
    )
        ...
        for move in self:
            for line in move.line_ids:
                if len(line.tax_ids) > 1:
                    errors.append(
                        _(
                            "Move Line %s has more than one tax "
                            "account, but allowed is only one." % line.name
                        )
                    )
                ...
                if not line.account_id.datev_automatic_account:
                    if line.tax_ids and not line.tax_ids[0].datev_tax_key:
                        errors.append(
                            _(
                                "Move line %s has taxes applied, but "
                                "the tax has no DATEV Tax Key" % line.name
                            )
                        )
    ...
    def _search_datev_ref(self, operator, value):
        ids = self.search([]).filtered(
            lambda x: x.datev_ref == value
            and x.payment_state in ["not_paid", "in_payment", "partial"]
        )
        return [("id", operator, ids.ids)]
