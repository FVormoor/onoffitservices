...
class Invite(models.TransientModel):
    ...
    def add_followers(self):
        ...
        for wizard in self:
            ...
            document.with_context(allow_manual_create=True).message_subscribe(
            ...
            if wizard.send_mail and wizard.message and not wizard.message == '<br>':
                message = self.env['mail.message'].create(
                    self._prepare_message_values(
                        document, model_name, email_from)
                )
                email_partners_data = []
                recipients_data = self.env['mail.followers']._get_recipient_data(
                    document, 'comment', False, pids=new_partners.ids)[document.id]
